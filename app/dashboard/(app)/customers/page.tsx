...
export default function CustomersPage() {
  ...
  const canonicalPlan = resolvePlanName(tenant?.plan)
  //const hasFullCustomerAccess = false
  const hasFullCustomerAccess =
    !!tenant &&
    (canonicalPlan === 'pro' || canonicalPlan === 'enterprise') &&
    hasBillingAccess(billing)

  const previewIds = useMemo(
    () => new Set(customers.slice(0, CUSTOMER_PREVIEW_LIMIT).map(c => c.id)),
    [customers]
  )

  function isLockedCustomer(c: any) {
    if (hasFullCustomerAccess) return false
    return !previewIds.has(c.id)
  }
  ...
  const totalLockedCustomers = hasFullCustomerAccess
    ? 0
    : Math.max(customers.length - CUSTOMER_PREVIEW_LIMIT, 0)

  const selectedLocked = selected ? isLockedCustomer(selected) : false
      ...
      >
        <div style={{ padding: '20px 20px 0', flexShrink: 0 }}>
          ...
          <p style={{ fontSize: '13px', color: T.t3 }}>
            {fetched ? `${customers.length} total` : 'Loading...'}
          </p>

          {!hasFullCustomerAccess && fetched && totalLockedCustomers > 0 && (
            <div
              style={{
                marginTop: '12px',
                background: T.card,
            ...
            >
              <div>
                ...
                >
                  Customer CRM preview
                </div>
                <div style={{ fontSize: '12px', color: T.t3, lineHeight: 1.5 }}>
                  You can access the first {CUSTOMER_PREVIEW_LIMIT} customers.{' '}
                  {totalLockedCustomers} more customer
                  {totalLockedCustomers === 1 ? '' : 's'} are locked until you
                  upgrade to Pro.
                </div>
              </div>
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {!fetched ? (
                  ...
                  >
                    <div
                      style={{
                        width: '120px',
                        height: '13px',
                        background: T.isDark ? '#222' : '#f0ede6',
                        borderRadius: '4px',
                        marginBottom: '6px',
                    ...
                    <div
                      style={{
                        width: '80px',
                        height: '11px',
                        background: T.isDark ? '#1e1e1e' : '#f5f2ee',
                        borderRadius: '4px',
                        animation: 'pulse 1.5s ease infinite',
                      }}
                    />
                ...
                filtered.map(c => {
                  const locked = isLockedCustomer(c)
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderBottom: `1px solid ${T.divider}`,
                        cursor: 'pointer',
                        background: selected?.id === c.id ? T.hover : 'transparent',
                            ...
                            {c.lead_source && (
                              <span
                                style={{
                                  ...
                                  padding: '1px 6px',
                                  borderRadius: '3px',
                                  background: T.isDark ? '#222' : T.hover,
                                  color: T.t3,
            ...
            {!selected ? (
              ...
              <div style={{ padding: '20px', maxWidth: '600px' }}>
                ...
                >
                  ...
                  >
                    You&apos;re currently seeing the free preview of your customer
                    list. Upgrade to Pro to access full customer records, booking
                    history, and CRM details beyond the first {CUSTOMER_PREVIEW_LIMIT}{' '}
                    customers.
                  </p>
