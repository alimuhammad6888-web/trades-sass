  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f8f6f1', fontFamily:'sans-serif', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden', marginTop:'16px' }}>
        {/* Left list */}
        <div style={{ width:'320px', flexShrink:0, borderRight:'1px solid #e8e4dc', display:'flex', flexDirection:'column', background:'#fff' }}>
          ...
          <div style={{ flex:1, overflowY:'auto' }}>
            {!fetched ? (
              ...
              <div key={c.id} onClick={() => selectCustomer(c)}
                style={{ padding:'14px 16px', borderBottom:'1px solid #f0ede6', cursor:'pointer', background:selected?.id===c.id?'#eef4fb':'transparent' }}>
                <div style={{ fontWeight:600, fontSize:'14px', color:'#1a1917', marginBottom:'3px' }}>{c.first_name} {c.last_name}</div>
                <div style={{ fontSize:'12px', color:'#9a9590', marginBottom:'2px' }}>{c.phone}</div>
                <div style={{ fontSize:'11px', color:'#c8c4bc' }}>{c.email}</div>
                {c.lead_source && <span style={{ fontSize:'10px', padding:'1px 6px', borderRadius:'3px', background:'#f0ede6', color:'#9a9590', fontFamily:'monospace', marginTop:'4px', display:'inline-block' }}>{c.lead_source}</span>}
              </div>
            ))}
          </div>
        </div>
          ...
          {!selected ? (
            ...
            <div style={{ maxWidth:'600px' }}>
              <div style={{ background:'#fff', border:'1px solid #e8e4dc', borderRadius:'8px', padding:'20px', marginBottom:'16px' }}>
                <h2 style={{ fontFamily:'Georgia, serif', fontSize:'22px', color:'#1a1917', marginBottom:'12px', fontStyle:'italic' }}>{selected.first_name} {selected.last_name}</h2>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  {[{label:'Phone',value:selected.phone},{label:'Email',value:selected.email},{label:'Source',value:selected.lead_source},{label:'Joined',value:fmtDate(selected.created_at)}].map(row => row.value && (
                    <div key={row.label}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px', marginBottom:'16px' }}>
                {[{label:'Total bookings',value:bookings.length},{label:'Completed',value:bookings.filter(b=>b.status==='completed').length},{label:'Total spend',value:'$'+(totalSpend/100).toFixed(0)}].map(s => (
              ...
              <div style={{ background:'#fff', border:'1px solid #e8e4dc', borderRadius:'8px', overflow:'hidden' }}>
                <div style={{ padding:'12px 16px', borderBottom:'1px solid #e8e4dc', fontFamily:'Georgia, serif', fontSize:'14px', fontStyle:'italic', color:'#1a1917' }}>Booking history</div>
                {bookings.length === 0 ? (
