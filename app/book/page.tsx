...
export default function BookPage() {
  ...
  async function submitBooking() {
    ...
    // Upsert customer
    const { data: existing } = await supabase.from('customers').select('id').eq('tenant_id', TENANT_ID).eq('phone', form.phone).single()

    let customerId: string

    if (existing) {
      customerId = existing.id
      await supabase.from('customers').update({ first_name:form.first_name, last_name:form.last_name, email:form.email }).eq('id', existing.id)
    } else {
    ...
    const ends   = new Date(starts.getTime() + selService!.duration_mins * 60000)
    ...
    const { data: booking } = await supabase.from('bookings').insert({
      ...
      price_cents: selService!.price_cents,
  ...
  const Logo = () => (
    <div style={header}>
      <span style={{ fontFamily:'Georgia,serif', fontSize:'20px', color:Y, fontStyle:'italic' }}>BigBoss Electric</span>
