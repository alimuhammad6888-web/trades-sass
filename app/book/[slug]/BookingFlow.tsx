...
type Tenant  = { id:string; name:string; slug:string; tagline?:string; primary_color?:string; accent_color?:string; booking_lead_time_hours?:number; booking_window_days?:number }
...
export default function BookingFlow({ slug, initialTenant, initialServices }: Props) {
  // Initialised directly from server-fetched props — no useEffect, no null flash
  ...
  // ── Styles ──────────────────────────────────────────────────────
  // brand is resolved from real tenant data — no default flash
  const Y = tenant?.primary_color || '#F4C300'  // solid fills
  const A = tenant?.accent_color  || Y           // text-color accents (fallback to primary)
  const BG     = '#0d0d0d'
  const CARD   = '#1a1a1a'
  const BORDER = '#2a2a2a'
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#0d0d0d', color: '#fff', fontFamily: 'sans-serif' }}>
        <div style={{ padding: '40px', textAlign: 'center' }}>
          <h2 style={{ color: Y, fontFamily: 'Georgia,serif' }}>
            {tenant.name}
          </h2>
