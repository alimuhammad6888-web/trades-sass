...
export default async function LandingPage({ params }: { params: { slug: string } }) {
  ...
  if (siteContent?.is_published === false && !isDev) {
    return (
      <div style={{ minHeight:'100vh', background:bg, color:text, fontFamily:tenantPublicChrome.fontSans, display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ textAlign:'center', maxWidth:'400px', padding:'40px 20px' }}>
          <div style={{ fontSize:'48px', marginBottom:'16px' }}>🚧</div>
          <h2 style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'28px', fontWeight:900, textTransform:'uppercase', marginBottom:'8px', color:text }}>{tenant.name}</h2>
          ...
          <a href={`/book/${slug}`} style={{ display:'inline-block', marginTop:'20px', padding:'12px 28px', background:brand, color:bg, fontFamily:tenantPublicChrome.fontDisplay, fontSize:'16px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', textDecoration:'none' }}>
            Book now
          </a>
        </div>
      </div>
    ...
    <>
      <style suppressHydrationWarning>{`
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        body { background:${bg}; color:${text}; font-family:${tenantPublicChrome.fontSans}; -webkit-font-smoothing:antialiased; }
        ...
        .hero-title { font-family:${tenantPublicChrome.fontDisplay}; font-size:clamp(64px,12vw,130px); font-weight:900; line-height:0.88; text-transform:uppercase; letter-spacing:-0.02em; animation:slide-up 0.8s ease both; color:${text}; }
        .book-btn { display:inline-flex; align-items:center; gap:10px; padding:16px 36px; background:${brand}; color:${bg}; font-family:${tenantPublicChrome.fontDisplay}; font-size:20px; font-weight:700; text-transform:uppercase; letter-spacing:0.1em; border:none; cursor:pointer; text-decoration:none; animation:slide-up 0.8s ease 0.4s both, pulse-glow 3s ease-in-out infinite; transition:transform 0.15s; }
        ...
        .service-name { font-family:${tenantPublicChrome.fontDisplay}; font-size:18px; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; color:${text}; line-height:1.1; }
        .service-desc { font-size:12px; color:${textMuted}; line-height:1.5; flex:1; }
        .service-price { font-size:13px; color:${accent}; font-weight:600; font-family:${tenantPublicChrome.fontDisplay}; letter-spacing:0.05em; }
        ...
        ${getTenantBrandMarkCss('.nav-logo', text)}
        ...
        .why-num { font-family:${tenantPublicChrome.fontDisplay}; font-size:64px; font-weight:900; color:${accent}; line-height:1; margin-bottom:8px; opacity:0.9; }
        .why-title { font-family:${tenantPublicChrome.fontDisplay}; font-size:20px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px; color:${text}; }
      ...
      <div style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', overflow:'hidden', paddingTop:'60px', background:bg }}>
        ...
        <div style={{ padding:'0 40px', maxWidth:'1100px', margin:'0 auto', width:'100%' }}>
          <div style={{ marginBottom:'16px', animation:'slide-up 0.6s ease both' }}>
            <span style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'clamp(13px,2vw,16px)', fontWeight:600, letterSpacing:'0.2em', textTransform:'uppercase', color:accent }}>
          ...
          <div style={{ position:'relative', display:'inline-block', marginTop:'12px', marginBottom:'36px' }}>
            <p style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'clamp(18px,3.5vw,32px)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', color:textMuted, animation:'slide-up 0.8s ease 0.15s both' }}>
        ...
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'20px', textAlign:'center' }}>
          {(stats.length > 0 ? stats : [
            ...
            <div key={i} style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
              <div style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'clamp(32px,5vw,48px)', fontWeight:900, color:bg, lineHeight:1 }}>{s.value || s.num}</div>
      ...
      <div id="services" style={{ background:bg, padding:'80px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:'40px', flexWrap:'wrap', gap:'16px' }}>
            <div>
              <div style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'12px', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.2em', color:accent, marginBottom:'8px' }}>What we do</div>
              <h2 style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'clamp(40px,7vw,72px)', fontWeight:900, textTransform:'uppercase', lineHeight:0.9, letterSpacing:'-0.01em', color:text }}>Our<br />Services</h2>
            </div>
          ...
          {services && services.length > 0 ? (
            ...
            <div style={{ border:`1px solid ${bgBorder}`, borderRadius:'12px', padding:'60px 40px', textAlign:'center' }}>
              <div style={{ fontSize:'48px', marginBottom:'16px' }}>🔧</div>
              <h3 style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'24px', fontWeight:700, textTransform:'uppercase', color:text, marginBottom:'8px' }}>Services coming soon</h3>
              ...
              {settings?.phone && (
                <a href={`tel:${settings.phone.replace(/[^\d]/g,'')}`} style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'14px 28px', background:brand, color:bg, fontFamily:tenantPublicChrome.fontDisplay, fontSize:'18px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', textDecoration:'none' }}>
                  📞 Call {settings.phone}
                </a>
              )}
            </div>
      ...
      <div id="why" style={{ background:bgDeep, padding:'80px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ marginBottom:'40px' }}>
            <div style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'12px', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.2em', color:accent, marginBottom:'8px' }}>Why choose us</div>
            <h2 style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'clamp(40px,7vw,72px)', fontWeight:900, textTransform:'uppercase', lineHeight:0.9, color:text }}>Why choose<br />{tenant.name}</h2>
          </div>
      ...
      <div className="cta-banner">
        <div style={{ maxWidth:'600px', margin:'0 auto' }}>
          <h2 style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'clamp(40px,8vw,80px)', fontWeight:900, textTransform:'uppercase', color:bg, lineHeight:0.9, marginBottom:'20px' }}>
      ...
      <div style={{ background:bgDeep, borderTop:`1px solid ${bgBorder}`, padding:'24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px' }}>
          <div style={{ fontFamily:tenantPublicChrome.fontDisplay, fontSize:'18px', fontWeight:900, textTransform:'uppercase', color:text }}>{tenant.name}</div>
