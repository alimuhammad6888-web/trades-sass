...
export default async function LandingPage({ params }: { params: { slug: string } }) {
  ...
  const [{ data: services }, { data: siteContent }] = await Promise.all([
    ...
    supabase
      .from('tenant_site_content')      // tenant_site_content, not site_content
  ...
  const accent = settings?.accent_color   || brand    // text-color accents on dark bg
    ...
    return (
      <div style={{ minHeight:'100vh', background:'#0a0a0a', color:'#fff', fontFamily:"'DM Sans',sans-serif", display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ textAlign:'center', maxWidth:'400px', padding:'40px 20px' }}>
          ...
          <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'28px', fontWeight:900, textTransform:'uppercase', marginBottom:'8px' }}>{tenant.name}</h2>
          <p style={{ color:'#555', fontSize:'14px', lineHeight:1.6 }}>Our website is coming soon. In the meantime, you can still book a service.</p>
          <a href={`/book/${slug}`} style={{ display:'inline-block', marginTop:'20px', padding:'12px 28px', background:brand, color:'#000', fontFamily:"'Barlow Condensed',sans-serif", fontSize:'16px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', textDecoration:'none' }}>
            Book now
          </a>
        </div>
      </div>
    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .hero-title { font-family:'Barlow Condensed',sans-serif; font-size:clamp(64px,12vw,130px); font-weight:900; line-height:0.88; text-transform:uppercase; letter-spacing:-0.02em; animation:slide-up 0.8s ease both; }
        .book-btn { display:inline-flex; align-items:center; gap:10px; padding:16px 36px; background:${brand}; color:#000; font-family:'Barlow Condensed',sans-serif; font-size:20px; font-weight:700; text-transform:uppercase; letter-spacing:0.1em; border:none; cursor:pointer; text-decoration:none; animation:slide-up 0.8s ease 0.4s both, pulse-glow 3s ease-in-out infinite; transition:transform 0.15s; }
        .book-btn:hover { transform:scale(1.03); }
        .grid-bg { position:absolute; inset:0; background-image:linear-gradient(rgba(244,195,0,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(244,195,0,0.03) 1px,transparent 1px); background-size:60px 60px; pointer-events:none; }
        ...
        .services-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:1px; background:#1a1a1a; border:1px solid #1a1a1a; border-radius:12px; overflow:hidden; }
        .service-card { background:#0f0f0f; padding:28px 24px; position:relative; overflow:hidden; transition:background 0.2s; display:flex; flex-direction:column; gap:10px; }
        ...
        .service-name { font-family:'Barlow Condensed',sans-serif; font-size:18px; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; color:#fff; line-height:1.1; }
        .service-desc { font-size:12px; color:#555; line-height:1.5; flex:1; }
        ...
        .nav { position:fixed; top:0; left:0; right:0; z-index:100; padding:0 40px; height:60px; display:flex; align-items:center; justify-content:space-between; background:rgba(10,10,10,0.95); backdrop-filter:blur(12px); border-bottom:1px solid #1a1a1a; }
        .nav-logo { font-family:'Barlow Condensed',sans-serif; font-size:22px; font-weight:900; text-transform:uppercase; letter-spacing:0.05em; color:#fff; }
        ...
        .nav-book { padding:8px 20px; background:${brand}; color:#000; font-family:'Barlow Condensed',sans-serif; font-size:14px; font-weight:700; text-transform:uppercase; letter-spacing:0.1em; text-decoration:none; transition:opacity 0.15s; }
        .nav-book:hover { opacity:0.88; }

        .why-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:1px; background:#1a1a1a; border:1px solid #1a1a1a; border-radius:12px; overflow:hidden; }
        .why-card { background:#0f0f0f; padding:32px 28px; }
        ...
        .why-title { font-family:'Barlow Condensed',sans-serif; font-size:20px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px; }
      ...
      <div style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', overflow:'hidden', paddingTop:'60px' }}>
        ...
        <div style={{ padding:'0 40px', maxWidth:'1100px', margin:'0 auto', width:'100%' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'24px', flexWrap:'wrap', animation:'slide-up 0.8s ease 0.3s both' }}>
            ...
            <a href={settings?.phone ? `tel:${settings.phone.replace(/[^\d]/g,'')}` : '#'} className="hero-phone-link" style={{ color:'#444', fontSize:'14px', textDecoration:'none', letterSpacing:'0.05em', animation:'slide-up 0.8s ease 0.4s both' }}>
        ...
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'20px', textAlign:'center' }}>
          {(stats.length > 0 ? stats : [
            ...
            <div key={i} style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
              <div style={{ fontFamily:'Barlow Condensed', fontSize:'clamp(32px,5vw,48px)', fontWeight:900, color:'#000', lineHeight:1 }}>{s.value || s.num}</div>
        ...
        <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:'40px', flexWrap:'wrap', gap:'16px' }}>
            <div>
              ...
              <h2 style={{ fontFamily:'Barlow Condensed', fontSize:'clamp(40px,7vw,72px)', fontWeight:900, textTransform:'uppercase', lineHeight:0.9, letterSpacing:'-0.01em' }}>Our<br />Services</h2>
            </div>
            <a href={`/book/${slug}`} className="view-all-link" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'12px 24px', border:'1.5px solid #2a2a2a', borderRadius:'4px', color:'#888', fontSize:'13px', fontWeight:500, textDecoration:'none', whiteSpace:'nowrap' }}>
              View all services →
            </a>
          </div>
          {services && services.length > 0 ? (
            ...
            <div style={{ border:'1px solid #1a1a1a', borderRadius:'12px', padding:'60px 40px', textAlign:'center' }}>
              <div style={{ fontSize:'48px', marginBottom:'16px' }}>🔧</div>
              <h3 style={{ fontFamily:'Barlow Condensed', fontSize:'24px', fontWeight:700, textTransform:'uppercase', color:'#fff', marginBottom:'8px' }}>Services coming soon</h3>
              <p style={{ fontSize:'14px', color:'#555', lineHeight:1.6, maxWidth:'400px', margin:'0 auto 24px' }}>We're setting up our online booking. Give us a call to schedule.</p>
              {settings?.phone && (
                <a href={`tel:${settings.phone.replace(/[^\d]/g,'')}`} style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'14px 28px', background:brand, color:'#000', fontFamily:'Barlow Condensed', fontSize:'18px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', textDecoration:'none' }}>
                  📞 Call {settings.phone}
                </a>
              )}
            </div>
      ...
      <div id="why" style={{ background:'#050505', padding:'80px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
          <div style={{ marginBottom:'40px' }}>
            ...
            <h2 style={{ fontFamily:'Barlow Condensed', fontSize:'clamp(40px,7vw,72px)', fontWeight:900, textTransform:'uppercase', lineHeight:0.9 }}>Why choose<br />{tenant.name}</h2>
          </div>
      ...
      <div className="cta-banner">
        <div style={{ maxWidth:'600px', margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:'12px', justifyContent:'center', flexWrap:'wrap' }}>
            <a href={`/book/${slug}`} className="cta-book-link" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'14px 32px', background:'#000', color:accent, fontFamily:'Barlow Condensed', fontSize:'18px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', textDecoration:'none' }}>
              ⚡ {siteContent?.cta_primary_text || 'Book now'}
            </a>
            <a href={settings?.phone ? `tel:${settings.phone.replace(/[^\d]/g,'')}` : '#'} style={{ display:'inline-flex', alignItems:'center', padding:'14px 32px', background:'transparent', color:'#000', border:'2px solid #000', fontFamily:'Barlow Condensed', fontSize:'18px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', textDecoration:'none' }}>
              {settings?.phone || 'Contact us'}
            </a>
          </div>
        </div>
      ...
      <div style={{ background:'#050505', borderTop:'1px solid #1a1a1a', padding:'24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px' }}>
          <div style={{ fontFamily:'Barlow Condensed', fontSize:'18px', fontWeight:900, textTransform:'uppercase' }}>{tenant.name}</div>
          <div style={{ fontSize:'12px', color:'#333' }}>{siteContent?.footer_tagline || settings?.tagline || 'Professional service you can trust.'}</div>
        </div>
      </div>
    </>
  )
