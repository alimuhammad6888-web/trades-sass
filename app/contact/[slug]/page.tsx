...
export default function ContactPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!tenant || !tenant.is_active) { setLoading(false); return }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: '24px', height: '24px', border: '2px solid #333', borderTopColor: '#F4C300', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
  ...
  if (!theme) {
    return (
      <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
  ...
  const inp: React.CSSProperties = {
    ...
    color: text, background: bgSurface,
  ...
  return (
    <>
      ...
      <nav className="contact-nav">
        <a href={`/t/${slug}`} className="contact-nav-logo">{tenantName}</a>
      ...
      <main style={{ minHeight: '100vh', background: bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '92px 20px 48px' }}>
        <div style={{ width: '100%', maxWidth: '500px', animation: 'slide-up 0.5s ease both' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            {logoUrl ? (
              <img src={logoUrl} alt={tenantName} style={{ height: '36px', objectFit: 'contain', marginBottom: '14px' }} />
            ) : (
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '26px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.04em', color: text, marginBottom: '10px' }}>
                {tenantName}
              </div>
            )}
            <div style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.18em', color: accent, marginBottom: '6px' }}>
              Get in touch
            </div>
          ...
          {success ? (
            <div style={{ background: bgSurface, border: `1px solid ${bgBorder}`, borderRadius: '12px', padding: '40px 32px', textAlign: 'center' }}>
              <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: `${brand}22`, border: `1.5px solid ${brand}55`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: '20px', color: brand }}>
                ✓
              </div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '22px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', color: text, marginBottom: '8px' }}>
                Message sent!
              </div>
              <p style={{ fontSize: '14px', color: textMuted, lineHeight: 1.6 }}>
            ...
            <div className="contact-form-card" style={{ background: bgSurface, border: `1px solid ${bgBorder}`, borderRadius: '12px', padding: '32px' }}>

              {error && (
                <div style={{ marginBottom: '20px', padding: '11px 14px', background: `${adjustHex(bg, 4)}`, border: `1px solid ${adjustHex(bg, 40)}`, borderLeft: `3px solid #ef4444`, borderRadius: '6px', fontSize: '13px', color: '#f87171' }}>
                  {error}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    <label style={lbl}>First name <span style={{ color: brand }}>*</span></label>
                    <input className="contact-input" style={inp} name="first_name" type="text" required autoComplete="given-name" value={form.first_name} onChange={handleChange} placeholder="Jane" />
                  </div>
                  <div>
                    <label style={lbl}>Last name <span style={{ color: brand }}>*</span></label>
                    <input className="contact-input" style={inp} name="last_name" type="text" required autoComplete="family-name" value={form.last_name} onChange={handleChange} placeholder="Smith" />
                  </div>
                </div>

                <div>
                  <label style={lbl}>Phone <span style={{ color: brand }}>*</span></label>
                  <input className="contact-input" style={inp} name="phone" type="tel" required autoComplete="tel" value={form.phone} onChange={handleChange} placeholder="+1 (555) 000-0000" />
                </div>

                <div>
                  <label style={lbl}>Email <span style={{ color: brand }}>*</span></label>
                  <input className="contact-input" style={inp} name="email" type="email" required autoComplete="email" value={form.email} onChange={handleChange} placeholder="jane@example.com" />
                </div>

                <div>
                  <label style={lbl}>Message <span style={{ color: brand }}>*</span></label>
                  <textarea className="contact-input" style={{ ...inp, resize: 'none' }} name="message" required rows={4} value={form.message} onChange={handleChange} placeholder="How can we help you?" />
                </div>
          ...
          <div style={{ marginTop: '20px', textAlign: 'center' }}>
            <a href={`/book/${slug}`} className="book-link">Ready to book? →</a>
          </div>
        </div>
      </main>
    </>
